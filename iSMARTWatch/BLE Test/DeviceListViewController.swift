...
//            println("- - - -"); println("sections \(indexPath.section) has \(tableView.numberOfRowsInSection(indexPath.section)) rows"); println("deleting \(attributePathArray.count) rows"); println("- - - -")
    ...
    func connectButtonTapped(sender: UIButton) {
        ...
        // Dashboard button
        let aaDash = UIAlertAction(title: "Dashboard", style: UIAlertActionStyle.Default) { (aa:UIAlertAction!) -> Void in
            self.connectInMode(ConnectionMode.Dash, peripheral: device.peripheral)
        }
        alertController.addAction(aaDash)
